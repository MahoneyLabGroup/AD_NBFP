---
title: SystemsSystem-level analysis of Alzheimer’s disease prioritizes candidate genes
...
#ADNI Results Directories
results.adni.dir <- here("adni")

results.adni.av.dir <- here("adni", "adni_av")
results.adni.hv.dir <- here("adni", "adni_hv")

results.adni.hippocampus.net.hv.dir <- here("adni", "adni_hv", "results_adni_hippocampus_net_hv")
results.adni.amygdala.net.av.dir <- here("adni", "adni_av", "results_adni_amygdala_net_av")

#MetaGWAS Results Directories
results.mwas.dir <- here("mwas")
...
The MAGMA output will have columns for gene, chromosome, start and stop positions, number of SNPs, N number of samples, Z statistic, and p-value.
...
Use gprofiler function gconvert to convert the gene entrez IDs from the MAGMA output to gene names. Then join the original data together with the converted gene names to get the final dataset to be used for down-stream processing.
...
## IV. Significant Genes

Find significant genes by filtering the gene list for genes with a p-value below 0.01. These genes will be used to train the SVM. (Note: only gene entrez IDs are needed for training)
...
We ran the SVM analysis separately for each of the gene lists pulled from the epilepsy GWAS paper (here we use JAE). The SVMs were trained to distinguish genes in the significant gene list from randomly chosen genes from outside the list. We use the function generate.triage.models() to accomplish this, which uses the `svm` function in the package `e1071` [@e1071]. The features used by the SVM are the edge weights between all genes and the genes in the JAE sig gene set. For each of the 100 SVM models, we used the JAE sig gene set genes as the true positives, while the true negatives were selected at random from outside of that gene set. Each true negative set was selected to have the same number of genes as were in the true positive set.
...
generate.triage.models(path = results.mwas.dir, 
...
```{r score_candidates}
score.candidates(results.mwas.dir, candidate.genes = mwas_genes$GENE)
```

## VI. Integrate Gene Information with SVM Output
...
mwas_amygdala_net_fin_tab <- 
  read_rds(here("mwas/mwas_amygdala_net_dx/results_mwas_amygdala_net_dx/Module1/Candidate.Gene.Results.rds"))

mwas_hippocampus_net_fin_tab <- 
  read_rds(here("mwas/mwas_hippocampus_net_dx/results_mwas_hippocampus_net_dx/Module1/Candidate.Gene.Results.rds"))
```


## VII. Gene Scores and Plotting
...
#1. Amygdala Volume | Amygdala Network
...
#2. Hippocampus Volume | Hippocampus Network
...
#### Compairing ADNI Volume GWAS P-values (Fig 2C)
```{r}
...
#### Compairing MWAS p-values to ADNI Volume GWAS p-values (Figs 2D and E)
```{r}
