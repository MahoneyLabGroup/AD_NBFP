...
SNP-level summary statistics were retrieved from the ADNI and Jansen et al. 2019 as stated above. We mapped SNP-level statistics to gene-level using Multi-marker Analysis of GenoMic Annotation [(MAGMA)](<https://ctg.cncr.nl/software/magma>). We used a 10kb annotation window around each gene and used the NCBI hg37 reference genome. LD estimations were done using the 1000 Genome's project CEU population. The MAGMA output will have columns for gene, chromosome, start and stop positions, number of SNPs, N number of samples, Z statistic, and p-value.
...
Find significant genes by filtering the gene list for genes with a p-value below 0.01. These genes will be used to train the SVM. (Note: only gene Entrez IDs are needed for training) These genes represent positional candidates.
