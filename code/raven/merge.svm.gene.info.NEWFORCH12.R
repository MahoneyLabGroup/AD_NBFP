#This function 
merge.svm.gene.info.NEWFORCH12 <- function(results.dir = ".", gene.info.table){
	...
	for(i in 1:length(module.dir)){
		...
		final.tableCH12 <- cbind(gene.info.table[gene.locale.table,], mean.svm[gene.locale.svm], mean.fp[gene.locale.svm])
		colnames(final.tableCH12)[c(6, 7)] <- c("Mean.SVM.Score", "Mean.FP.Rate")
		
		write.table(final.tableCH12, results.file, sep = ",", quote = FALSE, row.names = FALSE)
		
		mean.gene.position <- rowMeans(final.tableCH12[,c("start_position", "end_position")])
		
		jpeg(plot.file, height = 7, width = 10, units = "in", res = 300)
		plot.new()
		plot.window(xlim = c(min(mean.gene.position), max(mean.gene.position)), ylim = c(min(final.tableCH12[,"Mean.SVM.Score"]), max(final.tableCH12[,"Mean.SVM.Score"])))
		text(x = mean.gene.position, y = final.tableCH12[,"Mean.SVM.Score"], labels = final.tableCH12[,"external_gene_name"], cex = 0.7)
		axis(1);axis(2)
